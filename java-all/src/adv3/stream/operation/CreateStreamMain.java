...
public class CreateStreamMain {
    public static void main(String[] args) {
        ...
        List<String> list = List.of("a", "b", "c");   // 불변 리스트 (컬렉션)
        Stream<String> stream1 = list.stream();       // 리스트 -> 스트림
        stream1.forEach(System.out::println);

        System.out.println("2. 배열로부터 생성");
        String[] arr = {"a", "b", "c"};                // 문자열 배열 (불변)
        Stream<String> stream2 = Arrays.stream(arr);   // 배열 -> 스트림
