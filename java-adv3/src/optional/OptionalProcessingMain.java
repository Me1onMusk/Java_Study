...
public class OptionalProcessingMain {
    public static void main(String[] args) {
        ...
        optValue.ifPresent(v -> System.out.println("optValue 값: " + v));  //optValue 값: Hello
        ...
        optValue.ifPresentOrElse(
                v -> System.out.println("optValue 값: " + v),  // 값이 있으면 -> optValue 값: Hello
                () -> System.out.println("optValue는 비어있음")       // 값이 없으면
        );
        optEmpty.ifPresentOrElse(
                v -> System.out.println("optEmpty 값: " + v),  // 값이 있으면
                () -> System.out.println("optEmpty는 비어있음")       // 값이 없으면 -> optEmpty는 비어있음
        );

        // 값이 있으면 Function 적용 후 Optional로 반환, 없으면 Optional.empty()
        System.out.println("=== 3. map() ===");
        Optional<Integer> lengthOpt1 = optValue.map(String::length);
        System.out.println("optValue.map(String::length) = " + lengthOpt1);  // optValue.map(String::length) = Optional[5]
        Optional<Integer> lengthOpt2 = optEmpty.map(String::length);
        System.out.println("optEmpty.map(String::length) = " + lengthOpt2); // optEmpty.map(String::length) = Optional.empty
        ...
        System.out.println("optValue = " + optValue);    // optValue = Optional[Hello]
        System.out.println("nestedOpt = " + nestedOpt);  // nestedOpt = Optional[Optional[Hello]]
        ...
        System.out.println("optValue = " + optValue);          // optValue = Optional[Hello]
        System.out.println("flattenedOpt = " + flattenedOpt);  //flattenedOpt = Optional[Hello]
        ...
        System.out.println("filter(H) = " + filtered1); // filter(H) = Optional[Hello]
        System.out.println("filter(X) = " + filtered2); // filter(X) = Optional.empty

        // 값이 있으면 단일 요소 스트림, 없으면 빈 스트림
        System.out.println("=== 6. stream() ===");
        optValue.stream()
                .forEach(s -> System.out.println("optValue.stream() -> " + s));  // optValue.stream() -> Hello
